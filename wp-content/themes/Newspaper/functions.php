...
$tdnull['td_011_'] = 2;
$tdnull['td_011'] = base64_encode($fake_license_key);
$tdnull['td_011_check_time'] = time();
update_option('td_011', $tdnull);
update_option('td_cake_status', 2);

set_transient('TD_CHECKED_LICENSE', 'SUCCESS');
set_transient('TD_THEME_WELCOME', '1');
set_transient('TD_DOMAIN_ACTIVE', array('value' => true));

update_option('td_domain_validation', array(
    'envato_key' => $fake_license_key,
    'domains' => array(
        array(
            'ID' => 1,
...
update_option('td_license_status', array(
...
add_filter('pre_http_request', function($pre, $args, $url) {
    if ($url === 'https://cloud.tagdiv.com/api/templates/get_all' && 
        $args['method'] === 'POST' && !empty($args['body']) && isset($args['body']['wp_type'])) {
        $wp_type = sanitize_text_field($args['body']['wp_type']);
        $new_url = 'https://dl.gpltimes.com/file/gpltimes/newspaper/' . $wp_type . '.json';
        return wp_remote_get($new_url, ['timeout' => 60, 'sslverify' => false]);
    }
    
    if ($url === 'https://cloud.tagdiv.com/api/templates/check_domains') {
        global $tdnull;
        $fake_license_key = base64_decode($tdnull['td_011']);
        $current_domain = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'localhost';
        $future_date = date('Y-m-d H:i:s', strtotime('+15 days'));
    ...
    if ($url === 'https://cloud.tagdiv.com/api/templates/activate_domain') {
        return [
            'response' => ['code' => 200, 'message' => 'OK'],
            'body' => json_encode([
                'api_reply' => [
    ...
    if ($url === 'https://cloud.tagdiv.com/api/templates/check') {
        return [
            'response' => ['code' => 200, 'message' => 'OK'],
            'body' => json_encode([
                'key_is_valid' => true,
...
if( !function_exists('tagdiv_theme_css') ) {
    function tagdiv_theme_css() {
        wp_enqueue_style('td-theme', get_stylesheet_uri(), array(), filemtime( get_template_directory() . '/style.css' ) );
...
function sravan_logo_border_except_home() {
    if ( !is_front_page() && !is_home() ) {
        echo '<style>
        .tdi_32 .tdb-logo-a {
            display:flex !important;
            border:10px solid #0049a6 !important;
            padding:5px;
        }
        </style>';
    }
}
add_action('wp_head', 'sravan_logo_border_except_home');