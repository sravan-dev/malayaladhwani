...
<blockquote class="td_quote td_quote_left"><p>Fusce imperdiet, neque ut sodales dignissim, nulla dui. Nam vel tortor orci.</p></blockquote>
        ...
        $this->post_date = function ( $atts ) {
            ...
            $mad_reporter_date = get_post_meta( $post->ID, '_mad_reporter_date', true );

	        global $wp_version;

            if ( !empty( $mad_reporter_date ) ) {
                $mad_timestamp = strtotime( $mad_reporter_date );
                $post_date_array['date'] = date( DATE_W3C, $mad_timestamp );
                $post_date_array['time'] = date_i18n( $date_format, $mad_timestamp );
                $post_time_u = $mad_timestamp;
            } else {
	        // old WP
        ...
        $this->post_author = function ($atts) {
            ...
            $mad_reporter_name = get_post_meta( $post->ID, '_mad_reporter_name', true );
            $post_author_array['author_name'] = !empty( $mad_reporter_name ) ? $mad_reporter_name : get_the_author_meta( 'display_name', $post->post_author );
            $post_author_array['author_url']  = get_author_posts_url( $post->post_author );

            $mad_author_image_id = (int) get_post_meta( $post->ID, '_mad_author_image_id', true );
            if ( $mad_author_image_id ) {
                $mad_img_url = wp_get_attachment_image_url( $mad_author_image_id, 'thumbnail' );
                if ( ! $mad_img_url ) {
                    $mad_img_url = wp_get_attachment_image_url( $mad_author_image_id, 'full' );
                }
            }
            if ( ! empty( $mad_author_image_id ) && ! empty( $mad_img_url ) ) {
                $post_author_array['author_avatar'] = '<img src="' . esc_url( $mad_img_url ) . '" class="avatar avatar-' . esc_attr( $photo_size ) . ' photo" width="' . esc_attr( $photo_size ) . '" height="' . esc_attr( $photo_size ) . '" style="width:' . esc_attr( $photo_size ) . 'px;height:' . esc_attr( $photo_size ) . 'px;object-fit:cover;border-radius:0;" />';
            } else {
