...
function hook_wp_head() {
    // Set og:image for single posts and terms when no dedicated SEO plugin is active.
    ...
    if (!$td_class_exists) {
        if (is_single()) {
            ...
            if (!empty($post) && has_post_thumbnail($post->ID)) {
                ...
                if (!empty($td_image[0])) {
                    echo '<meta property="og:image" content="' . esc_url($td_image[0]) . '" />';
                    echo '<meta name="twitter:card" content="summary_large_image" />';
                    echo '<meta name="twitter:image" content="' . esc_url($td_image[0]) . '" />';
                }
            }
        }
...
function td_bottom_code() {
    ...
    <!--
        ...
        Theme: ' . TD_THEME_NAME . ' by tagDiv.com 2025
